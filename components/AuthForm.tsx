"use client";

import { useState } from "react";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import {
  Tabs,
  TabsContent,
  TabsList,
  TabsTrigger,
} from "@/components/ui/tabs";
import {
  signInWithEmail,
  signUpWithEmail,
  signInWithGoogle,
  sendMagicLink,
  resendConfirmation,
} from "@/app/login/actions";

interface AuthFormProps {
  error?: string;
  message?: string;
}

const NOT_CONFIRMED =
  "Your email isn't confirmed yet — check your inbox for a verification link.";

export default function AuthForm({ error, message }: AuthFormProps) {
  const [email, setEmail] = useState("");
  const showResend = error === NOT_CONFIRMED;

  return (
    <Card>
      <CardContent className="p-8">
        <h1 className="mb-1 text-2xl font-semibold">Welcome</h1>
        <p className="mb-6 text-sm text-muted-foreground">
          Sign in to track your subscriptions
        </p>

        {error && (
          <div className="mb-4 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700 dark:bg-red-950 dark:text-red-300">
            <p>{error}</p>
            {showResend && (
              <form action={resendConfirmation} className="mt-2">
                <input type="hidden" name="email" value={email} />
                <button
                  type="submit"
                  className="font-medium underline underline-offset-2 hover:text-red-800 dark:hover:text-red-200"
                >
                  Resend verification email
                </button>
              </form>
            )}
          </div>
        )}
        {message && (
          <p className="mb-4 rounded-lg bg-emerald-50 px-3 py-2 text-sm text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">
            {message}
          </p>
        )}

        {/* Google 登录 —— 两种方式通用 */}
        <form action={signInWithGoogle}>
          <Button type="submit" variant="outline" className="w-full">
            <svg className="h-4 w-4" viewBox="0 0 24 24">
              <path
                fill="#4285F4"
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              />
              <path
                fill="#34A853"
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              />
              <path
                fill="#FBBC05"
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"
              />
              <path
                fill="#EA4335"
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              />
            </svg>
            Continue with Google
          </Button>
        </form>

        <div className="my-5 flex items-center gap-3">
          <div className="h-px flex-1 bg-border" />
          <span className="text-xs text-muted-foreground">or</span>
          <div className="h-px flex-1 bg-border" />
        </div>

        <Tabs defaultValue="password">
          <TabsList className="mb-4 grid w-full grid-cols-2">
            <TabsTrigger value="password">Password</TabsTrigger>
            <TabsTrigger value="magic">Email link</TabsTrigger>
          </TabsList>

          {/* 密码登录 / 注册 */}
          <TabsContent value="password">
            <form className="space-y-3">
              <div className="space-y-1.5">
                <Label htmlFor="email">Email</Label>
                <Input
                  id="email"
                  name="email"
                  type="email"
                  required
                  placeholder="you@example.com"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="password">Password</Label>
                <Input
                  id="password"
                  name="password"
                  type="password"
                  required
                  minLength={6}
                  placeholder="••••••••"
                />
              </div>

              <div className="flex gap-2 pt-1">
                <Button formAction={signInWithEmail} className="flex-1">
                  Sign in
                </Button>
                <Button
                  formAction={signUpWithEmail}
                  variant="outline"
                  className="flex-1"
                >
                  Sign up
                </Button>
              </div>
            </form>
          </TabsContent>

          {/* Magic link 登录 */}
          <TabsContent value="magic">
            <form className="space-y-3">
              <div className="space-y-1.5">
                <Label htmlFor="magic-email">Email</Label>
                <Input
                  id="magic-email"
                  name="email"
                  type="email"
                  required
                  placeholder="you@example.com"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                />
                <p className="text-xs text-muted-foreground">
                  We&apos;ll email a one-time login link. Works for existing
                  accounts only.
                </p>
              </div>
              <Button formAction={sendMagicLink} className="w-full">
                Send login link
              </Button>
            </form>
          </TabsContent>
        </Tabs>
      </CardContent>
    </Card>
  );
}
